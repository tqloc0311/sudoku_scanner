...
class CameraView: UIView {
    ...
    private let centerView: UIImageView = .init(frame: .zero)
    ...
    private func setupLayout() {
        ...
        centerView.layer.zPosition = 999
    ...
    private func captureOutputHandler(buffer: CMSampleBuffer) {
        // video data
        guard let pixelBuffer: CVPixelBuffer = CMSampleBufferGetImageBuffer(buffer)
        else { return }
        
        //
        if let image = renderImage(from: pixelBuffer),
            let cropped = image.centerSquareCropping(percentage: 0.8) {
            
            delegate?.cameraView(self, didCapture: cropped)
        }
    }
...
extension CameraView: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ output: AVCaptureOutput,
        ...
        captureOutputHandler(buffer: sampleBuffer)
    }
}
